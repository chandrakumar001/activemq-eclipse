...
public class RouteView extends ViewPart implements ISelectionListener {
	...
	public static final String VIEW_ID = "com.googlecode.activemq.eclipse.views.RouteView";
	...
	public void createPartControl(Composite parent) {
		...
		HorizontalTreeLayoutAlgorithm hta = new HorizontalTreeLayoutAlgorithm(LayoutStyles.NO_LAYOUT_NODE_RESIZING);
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection instanceof StructuredSelection) {
					...
					if (firstElement instanceof DestinationNode) {
						DestinationNode node = (DestinationNode) firstElement;
		...
		fToggleLinkAction.setActionDefinitionId("com.googlecode.activemq.eclipse.views.RouteView.LinkAction");
		IHandlerService handlerService = (IHandlerService) getSite().getService(IHandlerService.class);
		handlerService.activateHandler("com.googlecode.activemq.eclipse.views.RouteView.LinkAction", new ActionHandler(fToggleLinkAction));
...
*/
	...
	private boolean containKnowIssues(String source) {
		...
		if (source.indexOf("@overwrite") != -1) {
			...
			String b = source.substring(source.indexOf("@overwrite") + "@overwrite".length(), source.length());

			source = a + b;
		}

		return (source.indexOf("pipeline") != -1) || (source.indexOf("multicast") != -1);
	...
	private class LinkAction extends Action {
		...
		public LinkAction() {
			super(RouteViewMessages.RouteView_action_toogleLinking_text, SWT.TOGGLE);

			setTitleToolTip(RouteViewMessages.RouteView_action_toggleLinking_toolTipText);
			LinkAction.this.setImageDescriptor(ImageShop.getDescriptor("synced.gif"));
